...
def prepare(project: Path, speech: bool):
    ...
    for relative in (
        "remotion", "output", "storyboard-final-timed.json", "render_plan.json",
        "cues.json", "composition-content.json", "narration-final.srt",
        "script-proposal.md", "sources.md", "example-audio.json",
    ):
    ...
    if not ffmpeg or (speech and not espeak):
        raise ValueError("install ffmpeg and espeak-ng (for --speech) before preparing the example")
    ...
    contract.update({key: value for key, value in source_contract.items() if key != "runtime_contract"})
    shutil.copytree(ROOT / "templates/narrated/remotion", project / "remotion", ignore=shutil.ignore_patterns("node_modules", ".cache", "out"))
    ...
    if speech:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for index, cue in enumerate(content["captions"]):
                ...
                subprocess.run([espeak, "-v", "en-us", "-s", "160", "-w", str(original), "--stdin"], input=cue["text"], text=True, check=True)
                segment = temporary / f"padded-{index}.wav"
                duration = (cue["endMs"] - cue["startMs"]) / 1000
                probe = subprocess.check_output([shutil.which("ffprobe") or "ffprobe", "-v", "error", "-show_entries", "format=duration", "-of", "csv=p=0", str(original)], text=True)
                rate = max(1.0, float(probe.strip()) / duration)
                run([ffmpeg, "-v", "error", "-i", str(original), "-af", f"atempo={rate},apad", "-t", str(duration), "-ar", "48000", "-ac", "1", str(segment)])
                segments.append(segment)
            arguments = [ffmpeg, "-v", "error"]
            for segment in segments:
                arguments += ["-i", str(segment)]
            inputs = "".join(f"[{index}:a]" for index in range(len(segments)))
            arguments += ["-filter_complex", f"{inputs}concat=n={len(segments)}:v=0:a=1[a]", "-map", "[a]", str(audio)]
            run(arguments)
        content["media"]["narration"] = {"path": "assets/demo-audio.wav", "kind": "local_audio", "label": "Original example text synthesized locally with eSpeak NG; pronunciation unreviewed"}
    else:
        run([ffmpeg, "-v", "error", "-f", "lavfi", "-i", "sine=frequency=220:sample_rate=48000:duration=24", "-af", "volume=0.025", str(audio)])
        content["media"]["narration"]["path"] = "assets/demo-audio.wav"
    (project / "remotion/src/content.json").write_text(json.dumps(content, ensure_ascii=False, indent=2) + "\n")
    (project / "composition-content.json").write_text(json.dumps(content, ensure_ascii=False, indent=2) + "\n")
    ...
    (project / "script-proposal.md").write_text("# From signal to decision\n\n" + "\n\n".join(cue["text"] for cue in content["captions"]) + "\n")
    (project / "sources.md").write_text("# Example sources\n\nOriginal conceptual text and vector graphics authored for Video Studio.\nAudio is generated locally; no private media or paid API is used.\nNo factual-reporting, pronunciation, human-review, or publication approval is claimed.\n")
    (project / "example-audio.json").write_text(json.dumps({"schema": "video_studio.example_audio.v1", "generator": "espeak-ng" if speech else "ffmpeg-sine", "sha256": hashlib.sha256(audio.read_bytes()).hexdigest(), "speech": speech, "pronunciation_reviewed": False}, indent=2) + "\n")
    contract_path.write_text(json.dumps(contract, indent=2) + "\n")
    return {"project": str(project), "speech": speech, "next": "npm ci inside remotion/, then use the MCP render-project runner"}


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("project", type=Path)
    parser.add_argument("--speech", action="store_true", help="use the local eSpeak NG engine; requires espeak-ng")
